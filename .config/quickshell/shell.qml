...
ShellRoot {
  ...
  PanelWindow {
    ...
    Rectangle {
      ...
      // special workspace wash — exactly like now-playing takeover: fills
      // the workspaces module gap-to-gap, inset by the pill's stroke, never
      // oversteps the pill's outer radius. Dimmed wash.
      readonly property real specialX: Zenon.padBar + workspacesMod.x - Zenon.gap
      readonly property real specialW: workspacesMod.width + Zenon.gap * 2
      Rectangle {
        id: specialTakeover
      ...
      Item {
        ...
        RowLayout {
          ...
          // The bell leads, then the updates count, then the tray: the same
          ...
          // Spacing that collapses with the thing it is spacing away from.
          // A plain Gap here would be left standing against the divider on a
          // day with no updates to report.
          Collapsible { active: updateMod.hasUpdates; openWidth: Zenon.gap }
          UpdateModule { id: updateMod; implicitHeight: Zenon.slot }
          TrayExpander { implicitHeight: Zenon.slot }
          // A plain Divider rather than a DividerSlot: this group used to be
          // able to empty out entirely, but the bell is always drawn — struck
          // through when there is nothing waiting — so there is always
          // something on its left to divide from the workspaces. It lands
          // after whichever of the three is last to collapse, which is what
          // puts it beside the updates count whenever that is showing.
