...
Scope {
  ...
  // Runs ONLY while the lock is deaf — locked, with not one surface holding
  // the keyboard — and stops itself the moment a surface takes it back, so
  // there is no cost in the healthy state and no polling to speak of.
  //
  // Resuming from suspend is where this happens. The outputs come back through
  // a modeset and the USB keyboards through a re-enumeration, both underneath a
  // lock that never went away, and the surface Qt had made active can return
  // with no focus item in it. Everything still paints; nothing you type lands.
  // Asking for the focus back is the only lever a client has here, so it asks
  // until it has it.
  Timer {
    id: earGuard
    interval: 250
    repeat: true
    running: sessionLock.locked && root.keyboardHere === 0
    onTriggered: {
    ...
    surface: WlSessionLockSurface {
      ...
      Item {
        ...
        Connections {
          target: root
          function onReclaim() {
            if (!catcher.activeFocus) catcher.forceActiveFocus();
          }
        }
