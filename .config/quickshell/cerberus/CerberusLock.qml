...
Scope {
  ...
  readonly property color clockFg: Zenon.cyan
  ...
  // ── motion ───────────────────────────────────────────────────────────
  // The lock fades up over the desktop instead of replacing it in one frame,
  // and fades back down before its surfaces are torn down, so both directions
  // read as a transition rather than a cut. Nothing is unlocked early: the
  // fade-out only starts once pam has already said yes.
  readonly property int fadeIn: 280
  readonly property int fadeOut: 220
  property bool covering: false
  // not readonly: a Behavior cannot attach to a read-only property, and this
  // one is still driven entirely by the binding below it
  property real reveal: root.covering ? 1 : 0
  Behavior on reveal {
    NumberAnimation {
      duration: root.covering ? root.fadeIn : root.fadeOut
  ...
  // Two steps, one frame apart: the surfaces have to exist before there is
  // anything to fade up, and `covering` set in the same frame as `locked`
  // would put them on screen already at full opacity.
  function engage() {
    sessionLock.locked = true;
    Qt.callLater(() => root.covering = true);
  }

  // Fade first, unmap second. The session stays locked for the length of the
  // fade, which is the point — tearing the surfaces down on the frame pam
  // returns is the "snap" that made unlocking feel like a stumble.
  function release() {
    root.covering = false;
    unlockDelay.restart();
  }
  ...
  Process {
    id: wipeProc
    command: ["sh", "-c", "rm -f " + root.shotDir + "/*.png " + root.shotDir + "/*.ppm"]
  }
  ...
  function lock() {
    ...
    shotProc.command = ["sh", "-c",
      "mkdir -p '" + root.shotDir + "'; " + grabs.join(" ") + " wait"];
      ...
      Item {
        ...
        Item {
          ...
          Row {
            ...
            // The whole row gives a little on each keystroke, so the feedback
            // carries even once the row is long enough that one more dot at
            // the end is easy to miss. Backspace gets it too — that is the
            // only acknowledgement a deletion has.
            property real kick: 1
            scale: dots.kick

            SequentialAnimation {
              id: kickAnim
              NumberAnimation { target: dots; property: "kick"; to: 1.06; duration: 70; easing.type: Easing.OutQuad }
              NumberAnimation { target: dots; property: "kick"; to: 1.0;  duration: 150; easing.type: Easing.OutBack }
            }
            ...
            Repeater {
              ...
              delegate: Text {
                ...
                // Each keystroke DROPS in and settles rather than appearing.
                // On a screen with no cursor and no echo, this animation is
                // the entire confirmation that the character was taken.
                scale: 0
                opacity: 0
                y: -field.height * root.dotsSize * 0.7
                ...
                Behavior on scale {
                  NumberAnimation { duration: 210; easing.type: Easing.OutBack }
                }
                Behavior on opacity { NumberAnimation { duration: 130 } }
                Behavior on y {
                  NumberAnimation { duration: 230; easing.type: Easing.OutBack }
                }
              }
            }
          }
        ...
        Item {
          ...
          Text {
            ...
            // Bold, not the ported Medium: seven-segment strokes are thin at
            // this size and the ghost behind needs the extra weight even more
            // than the live digits do.
        ...
        Text {
          ...
          color: root.dateFg
          // Deliberately NOT the clock's face: DSEG7 has only a seven-segment
          // approximation of an alphabet and no comma at all, so the date came
          // out as a shape rather than a sentence.
          font.family: root.fontFamily
          font.weight: root.fontWeight
          font.pointSize: root.dateSize
        }
