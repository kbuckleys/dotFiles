...
Item {
  ...
  property string childMenu: "" // "" | "apps" | "file" | "session"
  ...
  function onAppChosen(entry) {
    ...
    const isKitty = (entry.id && entry.id.toLowerCase() === "kitty.desktop") || (entry.name && entry.name.toLowerCase() === "kitty");
    if (isKitty) {
      Quickshell.execDetached(["sh", "-c", "gtk-launch '" + entry.id.replace(/'/g, "'\\''") + "' >/dev/null 2>&1 &"]);
      root.closeAll();
      return;
    }
    // diskord and any xdg-terminal-exec app must use kitty -1, not a new kitty instance
    if (entry.exec && entry.exec.includes("xdg-terminal-exec")) {
      const m = entry.exec.match(/-e\s+(.+)$/);
      const cmd = m ? m[1] : entry.exec;
      Quickshell.execDetached(["sh", "-c", "setsid kitty -1 --detach " + cmd + " >/dev/null 2>&1 &"]);
      root.closeAll();
      return;
    }
    if (entry.terminal) {
      const exec = entry.exec ? entry.exec.replace(/%[fFuUdDnNickvm]/g, "").trim() : "";
      if (exec) {
        Quickshell.execDetached(["sh", "-c", "setsid kitty -1 --detach " + exec + " >/dev/null 2>&1 &"]);
        root.closeAll();
        return;
      }
    }
  ...
  function onFileChosen(entry) {
    ...
    // file → open — .desktop files that wrap xdg-terminal-exec must use kitty -1
    if (String(entry.path).endsWith(".desktop")) {
      const p = String(entry.path);
      Quickshell.execDetached(["sh", "-c",
        "if grep -Eq 'xdg-terminal-exec|^Terminal=true' -- " + Icarus.shellQuote(p) + " 2>/dev/null; then " +
        "cmd=$(grep -m1 '^Exec=' -- " + Icarus.shellQuote(p) + " | sed 's/^Exec=//' | sed 's/%[fFuUdDnNickvm]//g' | sed -E 's/^ *xdg-terminal-exec[^ ]* *//; s/.*-e\\s*//; s/^[^ ]*kitty[^ ]*//'); " +
        "if [ -z \"$cmd\" ]; then cmd=$(grep -m1 '^Exec=' -- " + Icarus.shellQuote(p) + " | sed 's/^Exec=//' | sed 's/%[fFuUdDnNickvm]//g' | xargs); fi; " +
        "setsid kitty -1 --detach $cmd >/dev/null 2>&1 & " +
        "else xdg-open " + Icarus.shellQuote(p) + " >/dev/null 2>&1 & fi"
      ]);
      root.closeAll();
      return;
    }
        ...
        Repeater {
          ...
          delegate: Item {
            ...
            Rectangle {
              ...
              anchors.fill: parent
              radius: 0
              color: ((rootHover.containsMouse || (modelData.kind === "apps" && root.childMenu === "apps") || (modelData.kind === "file" && root.childMenu === "file") || (modelData.kind === "session" && root.childMenu === "session")) && !(modelData.isSeparator || false)) ? Zenon.hoverTint : "transparent"
            }
            ...
            Repeater {
              ...
              delegate: Item {
                ...
                Rectangle {
                  anchors.fill: parent
                  radius: 0
                  color: appHover.containsMouse ? Zenon.hoverTint : "transparent"
                }
