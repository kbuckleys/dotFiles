...
Item {
  ...
  property var sessionEntries: [
    // First, because the list below it climbs in severity and this is the
    // bottom of that scale — nothing is lost and the panels are back in a
    // second. `qs kill` before `qs -d`, in that order, in a shell that has
    // already been detached — so the replacement is never a child of the
    // instance it replaces.
    { id: "restart",    text: "Restart Shell", icon: "\uEBC7",
      cmd: "qs kill; sleep 0.4; qs -d", confirm: false },
