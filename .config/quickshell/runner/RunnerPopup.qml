...
PanelWindow {
  ...
  // Kept for anything that wants a whole number; the strip itself uses the
  // unrounded value, because rounding an animated width to integers is what
  // makes a smooth ease look like it is stepping.
...
property var statusbar: null
  ...
  function confirm() {
    ...
    if (row) {
      if (row.kind === "app") {
        popup.bump(row.key);
        const isKittyApp = row.key && String(row.key).toLowerCase() === "kitty";
        const isTerminalApp = !!row.terminal;
        const execHasXte = row.exec && String(row.exec).includes("xdg-terminal-exec");
        if (!isKittyApp && (isTerminalApp || execHasXte)) {
          const exec = row.exec ? String(row.exec).replace(/%[fFuUdDnNickvm]/g, "").trim() : "";
          if (exec) {
