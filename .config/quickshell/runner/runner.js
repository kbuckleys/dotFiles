...
function histPath() {
  return Paths.cacheDir() + "/runner-history";
}

function freqPath() {
  return Paths.cacheDir() + "/rofi-run-freq";
}
...
function terminalCommand(cmd) {
  ...
  return "xdg-terminal-exec --title=runner --hold -e " + shell() + " -i -c " + Strings.shellQuote(cmd) + " >/dev/null 2>&1 &";
}
...
function launchAppCommand(id) {
  return "gtk-launch " + Strings.shellQuote(id) + " >/dev/null 2>&1 &";
}
