...
PanelWindow {
  ...
  readonly property point srcPos: {
    ...
    const w = root.srcWin;
    const ox = w.anchors.left ? w.margins.left
                              : root.screen.width - w.margins.right - w.width;
    const oy = w.anchors.top ? w.margins.top
                             : root.screen.height - w.margins.bottom - w.height;
    const p = w.contentItem.mapFromItem(root.anchorItem, 0, 0);
    return Qt.point(ox + p.x, oy + p.y);
  }
