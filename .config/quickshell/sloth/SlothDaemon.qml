...
Scope {
  ...
  // for the bar.
  //
  // Any playing MPRIS player now counts as watching, whether windowed
  // or fullscreen — only paused is allowed to idle. Previously fullscreen
  // was also treated as watching to catch players that didn't report MPRIS,
  // but the requirement is now "any video playing, not just fullscreen".
  //
  ...
  // after play starts, not a second later. Check both isPlaying and
  // playbackState for players that only set one (browser vs mpv).
  ...
  // Fullscreen detection is kept for status reporting, but no longer
  // drives `watching` — any playing MPRIS player now counts, whether
  // windowed or fullscreen. Previously this was needed because mpv /
  // browsers could go fullscreen without appearing as Playing, but the
  // requirement is now "any video playing, only paused is allowed to
  // idle" so hasMprisPlaying/NowPlaying cover it.
  ...
  readonly property bool watching: root.inhibitOnPlayback
    && (NowPlaying.playing || root.hasMprisPlaying)
