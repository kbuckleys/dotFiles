...
// screen keeps it. A themed icon name is not a path and is not dropped: those
// are stable, and they are what the row falls back to anyway.
function localImage(img) {
  const s = String(img || "");
  return s.indexOf("/") === 0
    || s.indexOf("file://") === 0
    || s.indexOf("image://icon//") === 0;
}

function serialize(list) {
  return JSON.stringify(list.map((row) => {
    if (!localImage(row.image)) return row;
