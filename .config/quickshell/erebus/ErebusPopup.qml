...
PanelWindow {
  ...
  // eased copy of it — see the Math.min note in the other layers: without a
  // falling scalar of its own, handing the pill to another layer left erebus
  // fully opaque until its window blinked out.
  property real shownFade: popup.shown ? 1 : 0
  Behavior on shownFade {
    NumberAnimation { duration: Zenon.slow; easing.type: Zenon.ease }
  }
  readonly property real contentFade: popup.morphMode
    ? Math.min(popup.morphFade, popup.shownFade) : popup.shownFade
