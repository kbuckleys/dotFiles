...
PanelWindow {
  ...
  property var entries: [
    // the sleep lets erebus finish fading before heimdallr grabs the shot it
    // blurs, so the lock screen is the desktop and not erebus on its way out
    { id: "lockscreen", label: "", cmd: "sleep 0.35 && qs ipc call Heimdallr lock", confirm: false },
  ...
  readonly property real outerRadius: popup.morphMode ? popup.morphRadius : 6
  readonly property real outerRadiusLow: popup.morphMode ? popup.morphRadius : 10
