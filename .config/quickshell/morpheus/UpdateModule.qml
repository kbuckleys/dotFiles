...
Collapsible {
  ...
  function announce(o) {
    ...
    // announce() is reached.
    //
    // Colourless, though. On a toast the grouping and the arrows carry the
    // structure on their own, and the tooltip's inks are doing a job there that
    // they are not doing here. One builder, one strip — not two bodies to keep
    // in step.
    Quickshell.execDetached([
      "notify-send", "-a", "waybar-updates", "-u", "normal",
      root.countText + " " + word + " available",
      Helpers.stripMarkup(root.buildTooltip())
    ]);
  }
