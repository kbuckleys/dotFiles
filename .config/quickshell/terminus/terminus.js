...
function grepCommand(dir, query) {
  return "rg --hidden --no-ignore --files-with-matches --null --color=never -- "
    ...
    + " 2>/dev/null | sort -z | head -c 400000";
}
...
function bulkRenameCommand(names) {
  ...
  return "t=$(mktemp /tmp/terminus-rename.XXXXXX) || exit 1; "
    + "printf '%s\\n' " + lines + " > \"$t\"; "
    + "xdg-terminal-exec --title=terminus-rename -e sh -c "
...
// gio launch, NOT gtk-launch.
//
// gtk-launch runs the entry's Exec line and nothing more, so an entry that
// declares Terminal=true — nvim, htop, anything meant to be read in a terminal
// — is started with no terminal around it and dies immediately, silently,
// because the launch itself succeeded. Every row under "Open with" looked
// dead for exactly that reason. It is the same trap xdg-open set for opening
// files, and gio is the same way out: it honours Terminal=true and spawns one.
//
// gio wants the desktop FILE rather than the id, which is why appsCommand
// carries the path it already had in hand.
function openWithCommand(desktopFile, path) {
  return "gio launch " + Strings.shellQuote(desktopFile) + " "
...
// its own for the same reason extractCommand always did.
