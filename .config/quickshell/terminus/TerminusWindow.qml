...
FloatingWindow {
  ...
  readonly property real usageMax: {
    ...
    for (let i = 0; i < root.view.length; ++i) {
      const v = root.usageOf(root.view[i]);
      if (v > m) m = v;
    }
    return m;
  }
  ...
  function measureAll() {
    ...
    const todo = root.rows.filter((r) => r.isDir
      && root.dirSizes[r.path] === undefined && tried[r.path] !== true);
  ...
  Process {
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        statProc.command = ["sh", "-c", Terminus.statCommand(paths)];
  ...
  function search(mode, query) {
    ...
    searchProc.command = ["sh", "-c", mode === "grep"
      ? Terminus.grepCommand(root.cwd, query) : Terminus.findCommand(root.cwd, query)];
  ...
  FileView {
    ...
    // quickshell does offer — so a bookmark added in another terminus window, or
    ...
    // OUR OWN WRITE IS NOT NEWS, and it is recognised BY ITS CONTENT.
    //
    // setText lands on disk asynchronously and the fileChanged it causes
    // arrives while text() may still answer with what was there before — so a
    // blind re-read undid the change just made in memory and the sidebar
    // flicked back to the old list.
    //
    // The first attempt at this was a "we are writing" flag cleared on the
    // next change signal. That was worse: quickshell does not always emit one
    // for a write of our own, so the flag stayed up and swallowed the next
    // change somebody ELSE made — a bookmark added in another window, or by
    // hand, never appeared at all.
    //
    // Comparing against the text we last wrote has no such state to get stuck.
    // Identical content means there is nothing to do whoever wrote it, and
    // anything else is news.
    onFileChanged: {
      bookmarkFile.reload();
      if (String(bookmarkFile.text() || "") === root.bookmarkWrote) return;
      root.bookmarkWrote = "";
      root.loadBookmarks();
    }
  }

  property string bookmarkWrote: ""
  ...
  function editBookmarks(mutate) {
    ...
    root.bookmarkWrote = text;
    bookmarkFile.setText(text);
  }
  ...
  // Bumped every time something is opened. A counter rather than a signal
  // because a delegate can bind to a property and cannot connect to a signal
  // it has no reference to — see the note over openMenuAt for the same trap.
  property int openPulse: 0
  // and one for the cursor moving, for the same reason
  property int selPulse: 0

  // The sweep marks where the cursor CAME TO REST, not every row it passed
  // through on the way. Holding j down was firing one animation per row, which
  // is a stripe running down the whole list and says nothing; a single press or
  // a click still reads as immediate at this delay.
  Timer {
    id: selSettle
    interval: 70
    onTriggered: root.selPulse++
  }
  ...
  readonly property int markedCount: {
    let n = 0;
    for (const r of root.view) if (root.marked[r.path]) n++;
    return n;
  }
  ...
  function beginBulkRename() {
    ...
    bulkProc.command = ["sh", "-c", Terminus.bulkRenameCommand(root.bulkNames)];
  ...
  Process {
    ...
    stdout: StdioCollector {
      ...
      waitForEnd: true
      onStreamFinished: {
        const pairs = Terminus.bulkPairs(root.bulkNames, bulkOut.text);
  ...
  Item {
          ...
          Text {
            ...
            text: {
              ...
              if (n > 0) {
                ...
                return bytes > 0
                  ? n + " selected  \u00b7  " + Terminus.formatSize(bytes)
                  : n + " selected";
              }
    ...
    Rectangle {
      ...
      readonly property var groups: [
        ...
        ["panes", [["\\", "second pane on / off"],
                   ...
                   ["click", "step into the other side"]]],
    ...
    Item {
      ...
      anchors.fill: parent
      z: 20
      readonly property bool anyOpen: confirm.open || perms.open || props.open
    ...
    Item {
      ...
      readonly property var appItems: {
        ...
        for (let i = 0; i < apps.length; ++i) {
          // the desktop FILE, captured per iteration: gio launch takes a path,
          // and an entry the scan could not locate has nothing to run
          const file = apps[i].file;
          if (!file) continue;
          out.push({ label: apps[i].name, act: () => root.openWith(file, t.path) });
        }
        return out;
      }
      ...
      readonly property var items: {
        ...
        if (t.isDir)
          out.push({ label: "Open in new tab", key: "middle click",
        ...
        out.push({ label: "Sort by", key: ",", sub: menu.sortItems });
        out.push({ label: "Copy path", key: "c c", act: () => root.copyPath() });
        ...
        Column {
          ...
          Repeater {
            ...
            delegate: Item {
              ...
              // keyboard rather than competing with it. Right-aligned and
              // dimmed: it is a footnote to the entry, not a second label.
              Text {
                anchors.right: parent.right
                // clear of the chevron when there is one, so an entry that is
                // both a submenu and a key can say both
                anchors.rightMargin: modelData.sub ? 26 : 12
                anchors.verticalCenter: parent.verticalCenter
    ...
    Rectangle {
      ...
      ClippingRectangle {
        ...
        Column {
          ...
          // WHAT IT IS BEING APPLIED TO. The card used to open with a number
          // and nine boxes and nothing saying which file they were about,
          // which for a multi-selection is a question worth answering before
          // the mode is changed rather than after.
          Text {
  ...
  component Tile: Item {
    ...
    DragHandler {
      id: tileDrag
      target: null
      enabled: !!tile.entry && !tile.editing
      onActiveChanged: {
    ...
    TextInput {
      ...
      onVisibleChanged: {
        if (!visible) { tileEdit.hadFocus = false; return; }
  ...
  component EntryRow: Item {
    ...
    // A light passing across the bar as the cursor lands on it.
    //
    // The highlight used to appear fully formed, which at speed is hard to
    // follow: several rows all look equally like the one that just changed.
    // A sweep gives the change a direction and a moment, so the eye is
    // carried to it rather than having to find it.
    //
    // Driven by a COUNTER the window bumps when the cursor moves, not by
    // `current` changing. The list recycles its delegates, so `current` goes
    // true again whenever a row is reused for the cursor's index — which made
    // the sweep replay on every hover, every scroll and every return to a tab.
    // The cursor moving is the event; everything else is bookkeeping.
    Connections {
      target: root
      function onSelPulseChanged() {
    ...
    Row {
      ...
      Item {
        width: (parent.width - 24) * (entryRow.showMeta ? 0.44 : 1.0)
        height: parent.height
        ...
        TextInput {
          ...
          onVisibleChanged: {
            if (!visible) { entryEdit.hadFocus = false; return; }
          ...
          // Reset per EDIT, not per delegate: the list recycles its rows, so a
          // field that once had focus would otherwise carry that fact into the
          // next row it is reused for and cancel the moment anything blinked.
          property bool hadFocus: false
        }
      }
      ...
      Text {
        width: (parent.width - 24) * 0.14
        height: parent.height
        visible: entryRow.showMeta
        verticalAlignment: Text.AlignVCenter
        text: !entryRow.entry ? ""
          : (entryRow.entry.isDir ? "directory"
            : (entryRow.entry.isLink && entryRow.entry.broken ? "broken link"
              : Terminus.kindOf(entryRow.entry.name)))
        elide: Text.ElideRight
        color: Zenon.muted
        opacity: entryRow.dim ? 0.65 : 1
        font.family: "JetBrainsMono Nerd Font Propo"
      ...
      //
      // The number sits ON the bar, which is the arrangement worth keeping: a
      // bar and a caption beside it are two things to read, and one band with
      // its own value written in it is one. What the bar gains over the first
      // version is a TRACK — every row now shows the full length as well as
      // its own share, so a short bar is read against something instead of
      // floating in the dark, which is the difference between "small" and
      // "small compared to what".
      Item {
        id: sizeCell
        width: (parent.width - 24) * 0.18
        height: parent.height
        visible: entryRow.showMeta
        ...
        readonly property real frac: (sizeCell.on && root.usageMax > 0)
          ? Math.max(0, Math.min(1, root.usageOf(entryRow.entry) / root.usageMax))
          : 0
        ...
        readonly property bool biggest: sizeCell.on && root.usageMax > 0
          && root.usageOf(entryRow.entry) >= root.usageMax
        ...
        // Grows from the RIGHT, so it ends where the number ends and the two
        // share an edge rather than merely overlapping.
        Rectangle {
          id: usageFill
          visible: sizeCell.on
          anchors.right: usageTrack.right
          anchors.top: usageTrack.top
          anchors.bottom: usageTrack.bottom
          radius: usageTrack.radius
          // No artificial minimum. A floor of a few pixels made 6 B and 0 B
          // look like the same measurement with a rendering fault, and the
          // number beside it already says which is which — so a share too
          // small to draw is simply not drawn.
          width: (sizeCell.pending || sizeCell.frac <= 0) ? 0
            : Math.round(sizeCell.frac * usageTrack.width)

          // Lit along the top edge. A flat block at this height reads as a
          // slab; the sheen is what makes it read as a bar. Two stops, so no
          // gradient library is needed.
          //
          // Kept CLOSE TO THE TRACK in tone on purpose. The fill's edge lands
          // wherever the measurement puts it, which for most rows is somewhere
          // in the middle of the number — and against a bright fill that edge
          // cut the digits in half. Softened, the length still reads at a
          // glance while the number stays one piece of text on one surface.
          gradient: Gradient {
            GradientStop {
              position: 0.0
              color: sizeCell.biggest
                ? Qt.rgba(Zenon.sand.r, Zenon.sand.g, Zenon.sand.b, 0.30)
                : Qt.rgba(Zenon.cyan.r, Zenon.cyan.g, Zenon.cyan.b, 0.26)
            }
            GradientStop {
              position: 1.0
              color: sizeCell.biggest
                ? Qt.rgba(Zenon.sand.r, Zenon.sand.g, Zenon.sand.b, 0.17)
                : Qt.rgba(Zenon.cyan.r, Zenon.cyan.g, Zenon.cyan.b, 0.13)
            }
          }

          // The leading edge, brighter than the fill it ends.
          //
          // The fill stops wherever the measurement puts it, which for most
          // rows is somewhere inside the number — and an unmarked edge there
          // reads as the digits having been cut in half. A deliberate line
          // reads as a mark on a scale instead, which is what it is. Hidden at
          // the extremes, where there is nothing to mark: a full bar has no
          // interior edge and an empty one has no bar.
          Rectangle {
            anchors.left: parent.left
            anchors.top: parent.top
            anchors.bottom: parent.bottom
            width: 1
            visible: sizeCell.frac > 0.01 && sizeCell.frac < 0.995
            color: sizeCell.biggest
              ? Qt.rgba(Zenon.sand.r, Zenon.sand.g, Zenon.sand.b, 0.55)
              : Qt.rgba(Zenon.cyan.r, Zenon.cyan.g, Zenon.cyan.b, 0.5)
          }
        ...
        // Declared last so it draws over the bar. Brighter in the usage view
        // than the muted grey it uses elsewhere, because grey on a tinted
        // band is the one place that colour stops being readable.
        Text {
          anchors.fill: parent
          horizontalAlignment: Text.AlignRight
          verticalAlignment: Text.AlignVCenter
          ...
          text: {
            ...
            if (!e) return "";
            if (!e.isDir) return Terminus.formatSize(e.size);
          ...
          font.weight: sizeCell.on ? Font.Medium : Font.Normal
        }
      }
      ...
      Text {
        ...
        horizontalAlignment: Text.AlignRight
        verticalAlignment: Text.AlignVCenter
        text: entryRow.entry ? Terminus.formatTime(entryRow.entry.mtime) : ""
        color: Zenon.muted
        font.family: "JetBrainsMono Nerd Font Propo"
        font.pixelSize: Math.round(14 * root.zoom)
      }
  ...
  component ColHeadBar: Rectangle {
    ...
    Row {
      ...
      // THE FRACTIONS ARE OF THE INNER WIDTH, not of the whole row.
      //
      ...
      ColHead {
        width: (parent.width - 24) * (headBar.meta ? 0.44 : 1.0)
        label: "NAME"
        sortKey: "name"
      }
      ...
      ColHead {
        width: headBar.meta ? (parent.width - 24) * 0.14 : 0
        visible: headBar.meta
        label: "KIND"
        sortKey: "kind"
      }
      ColHead {
        width: headBar.meta ? (parent.width - 24) * 0.18 : 0
        visible: headBar.meta
      ...
      ColHead {
        width: headBar.meta ? (parent.width - 24) * 0.24 : 0
        visible: headBar.meta
        label: "MODIFIED"
        sortKey: "time"
        rightAlign: true
